from Vision_Unstacking_Robot import *
from Unstacking_Camera import *
from ConveyorMain import *
from Palletizing_Robot import *
import sys
from Palletizing_Camera import *
from Message import *

#请先阅读手册后，确定每个设备的串口号再运行此程序
if __name__=="__main__": 
    obj=Unstacking_Camera(cap_num=2,pump_x=0,pump_y=-24)
    cam=Palletizing_Camera(cap_num=1,pump_x=12,pump_y=18)
    robot=Vision_Unstacking_Robot("COM26",robot_speed=150)
    robot2=Palletizing_Robot("COM20",robot_speed=150,x_shift=10,y_shift=10)
    conveyer=ConveyorMain("COM21")
    
    
    while robot2.count<18:
        data=obj.detect()
        while len(data)<6:
            print("二维码检测数目不对，请确保二维码在相机范围内并且能正确识别")
            time.sleep(1)
            data=obj.detect()          
        
        for i in range(len(data)):
            robot.move(data[i][2],data[i][1]) 
                                 
            conveyer.open_conveyor(50)
            
            time.sleep(5)#如果传送带的供电电源是24V，延时改为5秒
            
            conveyer.close_conveyor()
            
            for i in range(3):
                try:
                    print("i=",i)                                
                    pose=cam.detect()
                    if pose is not None: 
                        break
                    
                except Exception as e:
                    if i==2:
                       message()                
                    temp=obj.exception_handling()
                    robot.Special_handling(temp[0][2],temp[0][1],temp[0][3])
                    conveyer.open_conveyor(100)
                    time.sleep(29)#如果传送带的供电电源是24V，延时改为5秒
                    conveyer.close_conveyor()
            id=robot2.move(pose[0],pose[1],pose[2],pose[3])                   
            robot2.judge(id)  

        
    
